...
if (isset($_POST['docsub'])) {
    ...
    if (!filter_var($demail, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($result) {
            ...
            header("Location: admin-panel1.php?msg=success");
            exit();
        } else {
...
if(isset($_POST['docsub1'])) {
  ...
  // Step 1: Cancel all future appointments of that doctor
  $updateAppointments = "UPDATE appointmenttb SET doctorStatus=0 WHERE doctor='$demail'";
  mysqli_query($con, $updateAppointments);

  // Step 2: Delete the doctor from doctb
  $query = "DELETE FROM doctb WHERE email='$demail'";
  $result = mysqli_query($con, $query);

  if($result) {
    echo "<script>alert('Doctor deleted and appointments updated.');</script>";
  } else {
...
.btn-primary{
  ...
  <div class="col-md-4" style="max-width:25%;margin-top: 3%;">
    <div class="list-group" id="list-tab" role="tablist">
      ...
      <a class="list-group-item list-group-item-action" href="#list-editdoc" id="list-editdoc-list" role="tab" data-toggle="list" aria-controls="home">Edit Doctor</a>
                  ...
                  <div class="panel panel-white no-radius text-center">
                    <div class="panel-body" >
                      ...
                      <p class="cl-effect-1">
                        ...
                        &nbsp| 
                        <a href="#list-editdoc" onclick="clickDiv('#list-editdoc-list')">Edit Doctors</a>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <tbody>
                  ...
                  ?>
                      <tr>
                        ...
                        <td>
                    <?php
$checkdoc = mysqli_query($con, "SELECT * FROM doctb WHERE username='".$row['doctor']."'");
$doctorExists = mysqli_num_rows($checkdoc) > 0;

if (!$doctorExists) {
    $appstatus = "Cancelled (Doctor Removed)";
...
<!-- EDIT DOCTOR TAB PANEL -->
<div class="tab-pane fade" id="list-editdoc" role="tabpanel" aria-labelledby="list-editdoc-list">
<form class="form-group" method="post" action="#list-editdoc">
    <div class="row">
      <div class="col-md-4"><label>Enter Doctor's Email to Edit:</label></div>
      <div class="col-md-8"><input type="email" class="form-control" name="edit_email" required></div><br><br>
    </div>
    <input type="submit" name="fetchdoc" value="Fetch Doctor" class="btn btn-primary">
  </form>

<?php
if(isset($_POST['fetchdoc'])) {
  $edit_email = $_POST['edit_email'];
  $query = "SELECT * FROM doctb WHERE email='$edit_email'";
  $result = mysqli_query($con, $query);
...
<form class="form-group" method="post" action="#list-editdoc">
      ...
      <div class="row">
        <div class="col-md-4"><label>Doctor Name:</label></div>
        <div class="col-md-8"><input type="text" class="form-control" name="doctor" value="<?php echo $row['username']; ?>" required></div><br><br>

        <div class="col-md-4"><label>Specialization:</label></div>
        <div class="col-md-8">
          <select name="special" class="form-control" required>
            <option value="General" <?php if($row['spec']=='General') echo 'selected'; ?>>General</option>
            <option value="Cardiologist" <?php if($row['spec']=='Cardiologist') echo 'selected'; ?>>Cardiologist</option>
            <option value="Neurologist" <?php if($row['spec']=='Neurologist') echo 'selected'; ?>>Neurologist</option>
            <option value="Pediatrician" <?php if($row['spec']=='Pediatrician') echo 'selected'; ?>>Pediatrician</option>
          </select>
        </div><br><br>

        <div class="col-md-4"><label>Email:</label></div>
        <div class="col-md-8"><input type="email" class="form-control" name="demail" value="<?php echo $row['email']; ?>" required></div><br><br>

        <div class="col-md-4"><label>Password:</label></div>
        <div class="col-md-8"><input type="text" class="form-control" name="dpassword" value="<?php echo $row['password']; ?>" required></div><br><br>

        <div class="col-md-4"><label>Consultancy Fees:</label></div>
        <div class="col-md-8"><input type="text" class="form-control" name="docFees" value="<?php echo $row['docFees']; ?>" required></div><br><br>
      </div>
      <input type="submit" name="updatedoc" value="Update Doctor" class="btn btn-success">
    </form>
<?php
  } else {
...
if (isset($_POST['updatedoc'])) {
  ...
  if (!filter_var($demail, FILTER_VALIDATE_EMAIL)) {
    ...
    // ✅ Perform update only if email is valid
    $query = "UPDATE doctb 
              SET username='$doctor', password='$dpassword', email='$demail', spec='$spec', docFees='$docFees' 
...
?>
  ...
  <script>
  // Preserve tab after post
  $(document).ready(function () {
    // Get hash from URL (e.g., #list-editdoc)
